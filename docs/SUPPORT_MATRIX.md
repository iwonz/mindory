...
| Local model installer runners | Supported baseline | Supported `LOCAL_MODEL_RUNNER_CATALOG` entries can be selected in the installer, resource-preflighted, started through Compose, installed with retry/log/report output and health-checked before migrations continue. `pnpm local-model:acceptance` covers the deterministic local HTTP runner, and `MINDORY_LOCAL_OCR_ACCEPTANCE_LIVE=true pnpm local-model:acceptance` covers the PaddleOCR runner. |
...
| OCR | Supported role and local runner | Scanned-PDF and image OCR run through `@mindory/llm` local HTTP or local-command OCR when enabled. The `paddleocr-pp-ocrv5-mobile` runner is supported through the `local-models-ocr` Compose profile and `MINDORY_LLM_OCR_LOCAL_HTTP_BASE_URL`. OpenAI-compatible OCR remains a custom provider path that requires explicit experimental mode. |
