...
| Local model installer runners | Supported baseline | Supported `LOCAL_MODEL_RUNNER_CATALOG` entries can be selected in the installer, resource-preflighted, started through Compose, installed with retry/log/report output and health-checked before migrations continue. `pnpm local-model:acceptance` covers the deterministic local HTTP runner, `MINDORY_LOCAL_OCR_ACCEPTANCE_LIVE=true pnpm local-model:acceptance` covers the Tesseract runner, `MINDORY_LOCAL_ASR_ACCEPTANCE_LIVE=true pnpm local-model:acceptance` covers the Faster Whisper runner, `MINDORY_LOCAL_VISION_ACCEPTANCE_LIVE=true pnpm local-model:acceptance` covers the image semantics runner, and `MINDORY_LOCAL_FACE_ACCEPTANCE_LIVE=true pnpm local-model:acceptance` covers the local face runner. |
