...
| Backup and restore | Supported MVP | `mindory-installer backup`, `backup-schedule`, `pitr-backup`, `pitr-restore`, `backup-archive`, `backup-upload`, `backup-download`, `backup-restore-archive` and `restore` cover config, installer metadata, PostgreSQL dumps, local object storage state, scheduled retention/health, local Compose PostgreSQL PITR and encrypted S3-compatible remote backup archives. External S3 RAW object bucket streaming is handled separately by `TASK-106`. |
