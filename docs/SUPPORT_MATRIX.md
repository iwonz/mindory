...
| Final public-ready gate | Supported gate | `pnpm public-ready:gate` dry-runs the final public pre-release checklist in `pnpm check`. Live mode runs from a fresh clone and combines published-release bootstrap, self-host live matrix, local-model acceptance, Web UI E2E, public stale wording validation and clean `git status --short`. |
| One-home installer layout | Supported baseline | `$MINDORY_HOME` config, logs, data, install and backups layout is implemented. |
| Installer supported multimodal preset | Supported baseline | `MINDORY_INSTALL_LOCAL_MODEL_PRESET=supported-multimodal` selects the checked local HTTP runners, requires resource confirmation, renders enabled `@mindory/llm` roles, exposes runner resource hints in the redacted summary and health-checks selected services before migrations. |
...
| Web UI | Supported local MVP surface | Web UI in `@mindory/ui` builds and runs as a static/proxy server with token/API URL entry, health banner, project/session navigation, message inspection, document upload, document list/detail, pipeline job progress, retry/reprocess controls, artifact source refs, unified search, context preview, manual memory creation, source-backed memory display, face identity list/rename/merge and runtime diagnostics through HTTP API calls. Docker Compose and installer deployments include the `ui` service on `MINDORY_UI_PORT` with `/api` proxy routing through `MINDORY_UI_API_URL`. `pnpm ui:e2e` dry-runs Playwright acceptance in `pnpm check`; live mode covers login, upload, jobs, artifacts/source refs, search, memory, context and desktop/mobile layout. |
...
| Local model installer runners | Supported baseline | Supported `LOCAL_MODEL_RUNNER_CATALOG` entries can be selected in the installer, resource-preflighted, started through Compose, installed with retry/log/report output and health-checked before migrations continue. `MINDORY_LOCAL_MODEL_ACCEPTANCE_LIVE=true pnpm local-model:acceptance` runs the full supported local-model gate; the narrower `MINDORY_LOCAL_OCR_ACCEPTANCE_LIVE`, `MINDORY_LOCAL_ASR_ACCEPTANCE_LIVE`, `MINDORY_LOCAL_VISION_ACCEPTANCE_LIVE` and `MINDORY_LOCAL_FACE_ACCEPTANCE_LIVE` flags run individual focused runner gates. |
...
| OCR | Supported role and local runner | Scanned-PDF and image OCR run through `@mindory/llm` local HTTP or local-command OCR when enabled. The `tesseract-local-ocr` runner is supported through the `local-models-ocr` Compose profile and `MINDORY_LLM_OCR_LOCAL_HTTP_BASE_URL`. OpenAI-compatible OCR remains a custom provider path that requires explicit experimental mode. |
| Vision captioning, object detection and image embeddings | Supported roles and local runner | Image vision captioning, object detection and image embeddings run through `@mindory/llm` local HTTP or local-command when enabled; image vectors are indexed through the selected vector backend. The `mindory-image-semantics-v1` runner is supported through the `local-models-vision` Compose profile plus `MINDORY_LLM_IMAGE_EMBEDDING_LOCAL_HTTP_BASE_URL` and `MINDORY_LLM_VISION_CAPTIONING_LOCAL_HTTP_BASE_URL`. |
| ASR | Supported role and local runner | Audio ASR runs through `@mindory/llm` local HTTP or local-command when enabled; embedded WAV transcript fallback remains supported. The `faster-whisper-tiny-asr` runner is supported through the `local-models-asr` Compose profile and `MINDORY_LLM_ASR_LOCAL_HTTP_BASE_URL`. |
| Face detection and recognition | Supported roles and local runner | Local HTTP and local-command face detection/recognition run through `@mindory/llm` when enabled; observations remain workspace-scoped and auto-matched by threshold. The `mindory-local-face-v1` runner is supported through the `local-models-face` Compose profile plus `MINDORY_LLM_FACE_DETECTION_LOCAL_HTTP_BASE_URL` and `MINDORY_LLM_FACE_RECOGNITION_LOCAL_HTTP_BASE_URL`. |
| Image and audio generation | Supported SDK adapter and smoke runner | OpenAI-compatible, local HTTP and local-command generation run through `@mindory/llm`; results validate expected MIME families, non-empty bytes and audit usage. The deterministic local HTTP runner returns valid PNG/WAV fixtures, and CLI smoke commands exercise both roles. |
