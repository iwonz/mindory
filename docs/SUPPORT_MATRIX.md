...
| Local model installer runners | Supported baseline | Supported `LOCAL_MODEL_RUNNER_CATALOG` entries can be selected in the installer, resource-preflighted, started through Compose, installed with retry/log/report output and health-checked before migrations continue. `pnpm local-model:acceptance` covers the deterministic local HTTP runner, and `MINDORY_LOCAL_OCR_ACCEPTANCE_LIVE=true pnpm local-model:acceptance` covers the Tesseract runner. |
