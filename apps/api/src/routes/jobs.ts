...
export interface JobRouteDependencies extends RouteDependencyOptions {
...
export async function registerJobRoutes(app: FastifyInstance, dependencies: JobRouteDependencies = {}): Promise<void> {
  assertRouteDependencies("Job routes", dependencies, [
    ["jobStore", dependencies.jobStore],
    ["jobDispatcher", dependencies.jobDispatcher]
  ]);

  app.get<{ Params: JobParams; Querystring: ProjectQuery }>("/v1/jobs/:id", async (request) => {
    const jobStore = requireRouteDependency(dependencies.jobStore, "jobStore");
    requireProjectPermission(request, request.query.projectId, "project:read");

    return toJobResponse(await jobStore.getJob(request.query.projectId, request.params.id));
  });
  ...
  app.get<{ Querystring: { projectId: string; status?: ProcessingJobStatus; type?: ProcessingJobType; limit?: number } }>("/v1/jobs", async (request) => {
    ...
    return {
      jobs: (await jobStore.listJobs(listInput)).map(toJobResponse)
    };
  });

  app.post<{ Params: JobParams; Body: ProjectQuery }>("/v1/jobs/:id/retry", async (request, reply) => {
    const jobDispatcher = requireRouteDependency(dependencies.jobDispatcher, "jobDispatcher");
    requireProjectPermission(request, request.body.projectId, "project:read");

    const enqueued = await jobDispatcher.retry(request.body.projectId, request.params.id);
    reply.status(202).send({
      job: toJobResponse(await jobDispatcher.store.getJob(request.body.projectId, request.params.id)),
      retry: {
