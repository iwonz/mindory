  ...
  try {
    ...
    await client.query(
      `
        ...
        values ($1, $2, $3::jsonb)
      `,
      [input.projectId, "Integration Test Project", JSON.stringify({ test_run_id: testRunId })]
    );
    await client.query(
      `
      ...
      `,
      [input.tokenId, input.projectId, "Integration bootstrap token", hashAccessToken(input.token), JSON.stringify({ test_run_id: testRunId })]
    );
    await client.query(
      `
