...
assert(cliPackage.dependencies?.["@mindory/llm"] === "workspace:*", "@mindory/cli must depend on @mindory/llm for model diagnostics.");
...
assert(commands.includes("llm generate-image"), "CLI help must document image generation diagnostics.");
assert(commands.includes("llm generate-audio"), "CLI help must document audio generation diagnostics.");
...
assert(commands.includes("buildMindoryLlm"), "CLI generation diagnostics must use @mindory/llm.");
assert(commands.includes("runtime.disabledResult"), "CLI generation diagnostics must preserve disabled role behavior.");
assert(commands.includes("sha256"), "CLI generation diagnostics must summarize generated bytes without printing them by default.");
...
for (const token of [
  ...
  "llm\", \"generate-audio\"",
  "document\", \"upload\"",
  "MINDORY_LLM_IMAGE_GENERATION_PROVIDER",
