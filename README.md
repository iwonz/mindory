...
| Installer | Supported today: wizard, plan/dry-run, prepare execution for `$MINDORY_HOME` directories/config/compose assets, Docker Compose startup through health checks, first project/token provisioning, dependency detection, lock/journal diagnostics, bootstrap staging and installer acceptance. Future: update, uninstall and full resume execution. |
